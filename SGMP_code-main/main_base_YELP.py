...
def get_args():
    ...
    parser.add_argument('--num_layers', type=int, default=3)   # numero di volte in cui blocco del codice viene fatto girare/ per epoca
...
def main(data, args):
    ...
    if args.dataset == 'synthetic' and 'brain':    # aggiungo and
        ...
        input_channels_node, hidden_channels, readout = 957, 64, args.readout
    else:
        input_channels_node, hidden_channels, readout = 9, 64, args.readout
    
        
    
    if args.dataset in [ 'BACE', 'BBBP','YELP']:
        task = 'classification'                  
