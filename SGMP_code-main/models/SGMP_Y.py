...
class SGMP(torch.nn.Module):
    ...
    def forward(self, x, pos, batch, edge_index_3rd=None):  
        ...
        # extract node types from their z-coordinate
        
        node_types = torch.sign(pos[:, 2])

        # filter edge_index_3rd based on node types
        
        i, j, k, p = edge_index_3rd
        mask = node_types[j] != node_types[i]
        ...
        # calculate distances, angles and cross products
